...
import { BlogService, PlatformService } from '@nicecactus-platform/graph-ql-service';
        ...
        this.actions$.pipe(
            ...
            switchMap(({ id }) =>
                this.userService.load(id).pipe(
                    map(({ Blog, currencies, languages }) =>
                        loadUserSuccess({ Blog, userDependencies: { currencies, languages } })
                    ),
        ...
        this.actions$.pipe(
            ...
            switchMap(() =>
                forkJoin([this.userService.userFactory(), this.userService.loadDependencies()]).pipe(
                    map(([Blog, dependencies]) => createUserSuccess({ Blog, userDependencies: dependencies })),
